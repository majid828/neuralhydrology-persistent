# neuralhydrology/datasetzoo/fifteenminutecamelsus.py
...
class FifteenMinuteCamelsUS(camelsus.CamelsUS):
    """Dataset providing 15-minute data for CAMELS US basins.

    Design:
    - Dynamic forcings are available as hourly (AORC/NLDAS). We upsample to 15-min by forward-fill.
    - Precipitation APCP_surface is an hourly accumulation -> split into four 15-min chunks (divide by 4).
    - Target discharge is loaded as true 15-min USGS QObs (parameter 00060) from RDB files.
    """
    ...
    def __init__(self,
        ...
        super(FifteenMinuteCamelsUS, self).__init__(cfg=cfg,
                                                   is_train=is_train,
                                                   period=period,
                                                   basin=basin,
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        """Load 15-min inputs (from hourly forcings) and true 15-min outputs (QObs)."""
        ...
        for forcing in self.cfg.forcings:
            if forcing.endswith('_hourly'):
                # For 15-min dataset: DO NOT load hourly discharge here.
                df = self.load_hourly_data(basin, forcing, load_discharge=False)
            else:
        ...
        # (mostly irrelevant here since we didn't load hourly qobs, but keep for safety)
        # ------------------------------------------
        ...
        # ==========================================================
        # 6) Always convert forcings to 15-min and load true 15-min QObs
        # ==========================================================
...
# ==========================================================
# Original hourly CAMELS-US helpers (still needed)
# ==========================================================
